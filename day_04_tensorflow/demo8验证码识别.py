...
def init_weight(shape):
    ...
    :return:
    """
    return tf.Variable(tf.random_normal(shape, mean=0.0, stddev=1.0))
...
def fc_model(image):
    ...
    with tf.variable_scope("model"):
        ...
        # 随机初始化权重偏置
        weight = init_weight([20 * 80 * 3, 4 * 26])
        bias = init_bias(([100, 4 * 26]))
        # 进行全连接层计算[100, 4 * 26]
...
def predict_to_onehot(label):
    """
    将图片的目标值转换成onehot编码
    :param label: [100,4]=>[[13,25,4,3],[24,1,3,2]......]
    :return:
    """
    # 进行one_hot编码转换，提供给交叉熵损失计算，准确率计算[100, 4, 26]
    return tf.one_hot(label, depth=FLAGS.letter_num, on_value=1.0, axis=2)
